...
class FileController extends BackendController
{
    public function index(Request $request)
    {
        ...
        if (!$request->ajax() || ($request->has('modal') && $request->modal == 1)) {
            ...
            $type = config('cms.backend.media.type.file');
            ...
            if (!$modal) {
                ...
                if ($source != 'ckeditor') {
                    ...
                    return view('backend.media.ckeditor', compact('type', 'typeName', 'arrListMedia', 'label', 'date_from', 'date_to', 'upload_config', 'multi', 'source', 'editor_name', 'CKEditorFuncNum', 'pagination', 'item'));
                }
            }
        } else {
            $label = $request->label ?? [];
            ...
            return view('backend.media.file.index', compact('arrListMedia', 'type', 'typeName', 'label', 'date_from', 'date_to', 'upload_config', 'pagination', 'item'));
        }
    }

    public function edit(Business_Media $fileInfo)
    {
        return view('backend.media.file.edit', compact('fileInfo'));
    }

    public function download(Business_Media $fileInfo)
    {
        $arrInfo = json_decode($fileInfo->media_info, true);

        return response()->download(file_url($fileInfo->media_filename), $fileInfo->media_filename, [
            'Content-Type' => $arrInfo['mimetype']
        ]);
    }
}
